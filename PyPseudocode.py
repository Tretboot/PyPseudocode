...
def matrix_run():
    ...
    # Setze die Richtung auf "rechts"
    direction = "right"
    
    # Setze die Anzahl der Leerzeichen am Anfang jeder Zeile auf 0
    spaces = 0
    ...
    while True:
        ...
        for line in code.split("\n"):
            text.insert(END, " " * spaces + line + "\n")
            text.see(END)
            time.sleep(0.1)
        
        # Ändere die Richtung und die Anzahl der Leerzeichen je nach aktueller Richtung
        if direction == "right":
            spaces += 1
            if spaces >= 10:
                direction = "left"
...
# Erstelle einen "?" Button der Informatinen über das Programm anzeigt.
from tkinter import messagebox

def zeige_info():
    info_text = "Programmed by Tretboot with the help of Ai\n\nLicense: The Unlicense\n\n"
    lizenz_text = """This is free and unencumbered software released into the public domain.
...
For more information, please refer to <https://unlicense.org>"""
    messagebox.showinfo("Information", info_text + lizenz_text)

info_button = tkinter.Button(fenster, text="?", command=zeige_info)
